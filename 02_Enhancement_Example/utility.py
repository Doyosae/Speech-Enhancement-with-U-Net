...
"""
def get_realData (data, data_length, wanted_size = 256):
    
    for index in range (data_length):
        data[index] = data[index][:wanted_size, :wanted_size]
    
    data = np.reshape(data, (data_length, wanted_size, wanted_size, 1))
    data = data.real
...
def get_imagData (data, data_length, wanted_size = 256):
...
def createIdealBinaryMask (clean_data, 
    ...
    testImage = np.reshape(idealBinaryMask, (data_length, image_size, image_size))
...
def createIdealRatioMask (clean_data, 
    ...
    parts2 = np.abs(calen_data)**2 + np.abs(mixture_data)**2
    ...
    testImage = np.reshape(idealRatioMask, (data_length, image_size, image_size))
...
"""
def showMask (mask, figure_count = 5, image_size = 256, 
              cmap = "gray", title = "title"):

    fig, ax = plt.subplots(1, figure_count, figsize = (20, 20))
    for index in range(figure_count):

        ax[index].imshow(np.reshape(mask[32*index], (image_size, image_size)), cmap = cmap)
...
def showSpectogram (data, title, 
                    image_size = 256, fig_size = (10, 7)):
    
    output = np.reshape(data, [image_size, image_size])
